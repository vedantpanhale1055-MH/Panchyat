...
const confirmDelete = (onConfirm: () => void) => {
  if (Platform.OS === 'web') { if (window.confirm('Delete this complaint?')) onConfirm(); }
  else {
...
export default function ComplaintsScreen() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'societies', SOCIETY_ID, 'complaints'), orderBy('createdAt', 'desc'));
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addDoc(collection(db, 'societies', SOCIETY_ID, 'complaints'), {
        title: title.trim(), description: description.trim(), urgency,
        status: 'Pending', userId: auth.currentUser?.uid,
        userName: user?.name, flatNo: user?.flatNo, wing: user?.wing,
        upvotedBy: [], downvotedBy: [], createdAt: new Date().toISOString(),
      });
  ...
  const urgencyColor = (u: string) => u === 'High' ? colors.danger : u === 'Medium' ? colors.warning : colors.success;
  const statusColor = (s: string) => s === 'Resolved' ? colors.success : s === 'In Progress' ? colors.warning : colors.primary;
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      <View style={[styles.header, { backgroundColor: colors.header, borderBottomColor: colors.border }]}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity style={[styles.addBtn, { backgroundColor: colors.primary }]} onPress={() => setShowForm(true)}>
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.card }]}>
            ...
            <TextInput style={[styles.input, { backgroundColor: colors.input, borderColor: colors.border, color: colors.text }]} placeholder="Short title" placeholderTextColor={colors.muted} value={title} onChangeText={setTitle} />
            <Text style={[styles.label, { color: colors.subtext }]}>Description</Text>
            <TextInput style={[styles.input, styles.textArea, { backgroundColor: colors.input, borderColor: colors.border, color: colors.text }]} placeholder="Describe the issue..." placeholderTextColor={colors.muted} value={description} onChangeText={setDescription} multiline numberOfLines={4} />
            <Text style={[styles.label, { color: colors.subtext }]}>Urgency</Text>
            <View style={styles.urgencyRow}>
              {URGENCY_LEVELS.map((u) => (
                <TouchableOpacity key={u} style={[styles.urgencyBtn, { borderColor: urgency === u ? urgencyColor(u) : colors.border, backgroundColor: urgency === u ? urgencyColor(u) + '20' : 'transparent' }]} onPress={() => setUrgency(u)}>
      ...
      {loading ? <ActivityIndicator color={colors.primary} style={{ marginTop: 40 }} /> : (
        <ScrollView contentContainerStyle={styles.list}>
          {complaints.length === 0 && <Text style={[styles.emptyText, { color: colors.subtext }]}>No complaints filed yet.</Text>}
          {complaints.map((item) => {
            const uid = auth.currentUser?.uid;
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => router.push({ pathname: '/complaint-detail' as any, params: { id: item.id, title: item.title, description: item.description, urgency: item.urgency, status: item.status, userName: item.userName, flatNo: item.flatNo } })}
                activeOpacity={0.85}
              >
                ...
                <View style={styles.cardContent}>
                  ...
                  <View style={styles.cardFooter}>
                    <Text style={[styles.cardMeta, { color: colors.muted }]}>{item.userName} • {item.wing}-{item.flatNo}</Text>
                    <View style={styles.footerActions}>
                      <TouchableOpacity style={[styles.voteBtn, { borderColor: hasUpvoted ? colors.success : colors.border }, hasUpvoted && { backgroundColor: colors.success + '18' }]} onPress={() => handleVote(item, 'up')}>
                        <Text style={[styles.voteArrow, { color: hasUpvoted ? colors.success : colors.muted }]}>▲</Text>
                        <Text style={[styles.voteCount, { color: hasUpvoted ? colors.success : colors.subtext }]}>{upvotedBy.length}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[styles.voteBtn, { borderColor: hasDownvoted ? colors.danger : colors.border }, hasDownvoted && { backgroundColor: colors.danger + '18' }]} onPress={() => handleVote(item, 'down')}>
                        <Text style={[styles.voteArrow, { color: hasDownvoted ? colors.danger : colors.muted }]}>▼</Text>
                        <Text style={[styles.voteCount, { color: hasDownvoted ? colors.danger : colors.subtext }]}>{downvotedBy.length}</Text>
                      </TouchableOpacity>
                      {canDelete && (
                        <TouchableOpacity onPress={() => handleDelete(item.id, item.userId)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 14, marginBottom: 12, borderWidth: 1, overflow: 'hidden', flexDirection: 'row', elevation: 1 },
  ...
  voteBtn: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, borderWidth: 1 },
